...
- **Barbarian Inner Fire bar restored** (`Forms/FormMain.Designer.cs`) — Both `ComponentBarsHealth` and `ComponentBarsMana` were added to cell `(0, 0)` of the 5-column `TableLayoutPanelBars`. When two controls claim the same cell in a `TableLayoutPanel`, the last one added wins — Health was added last, so it claimed column 0 and the Mana/Inner Fire bar was displaced off the visible area. Fixed by assigning each bar to its correct column: Health→0, Mana/Inner Fire→1, Concentration→2, Stamina→3, Spirit→4.
